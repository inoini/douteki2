...
public class PostDAO {
	...
	// 投稿保存
	public void insertPost(String name, String message, String sns_url,String sns_url2, String discordName, boolean isAdmin) {

		System.out.println("DAO呼ばれた");

		try {
			Connection conn = DBUtil.getConnection();

			String sql = "INSERT INTO posts(name, message, sns_url, sns2, discord_name,is_admin) VALUES (?, ?, ?, ?, ?,?)";
			PreparedStatement ps = conn.prepareStatement(sql);

			ps.setString(1, name);
			ps.setString(2, message);
			ps.setString(3, sns_url);
			ps.setString(4, sns_url2);
...
//消去メソッド
	...
	public int countPostsToday(String userId) {
	    ...
	    String sql =
	        ...
	        "WHERE name = ? AND DATE(created_at) = CURDATE()";
